...
import { listRiders } from '../../../services/platform/ridesPlatformService';

interface RiderUsersPageProps {
  onOpenUser: (userId: string) => void;
}

export function RiderUsersPage({ onOpenUser }: RiderUsersPageProps) {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['platformRiderUsers', token, q, page],
    queryFn: () =>
      listRiders(token!, {
        q: q.trim() || undefined,
        page,
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-200 dark:bg-slate-900/90 dark:border-slate-800">
              <tr className="text-left text-xs text-slate-500 uppercase tracking-wider">
                ...
                <th className="px-4 py-3 font-medium hidden lg:table-cell">Trips</th>
              ...
              {rows.map((r) => (
                <tr key={r.user_id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                  ...
                  <td className="px-4 py-3 hidden md:table-cell">
                    <span
                      className={`text-xs ${
                        r.account_status === 'active' ? 'text-emerald-400' : 'text-red-400'
                      }`}
                    >
                      {r.account_status}
                    </span>
                  </td>
                  <td className="px-4 py-3 hidden lg:table-cell text-slate-400 text-xs">{r.total_trips}</td>
