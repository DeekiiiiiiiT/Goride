...
import {
  ...
  DASH_CUSTOMER_DATA_SAFETY_TEMPLATE_VERSION,
  DASH_CUSTOMER_PLAY_STORE_CATALOG,
  DASH_CUSTOMER_PLAY_STORE_META,
...
const BASE = `${API_ENDPOINTS.delivery}/admin/play-store`;
...
function mapPayload(data: ApiPayload): PlayStoreLaunchPayload {
  ...
  return {
    meta: DASH_CUSTOMER_PLAY_STORE_META,
    catalog: DASH_CUSTOMER_PLAY_STORE_CATALOG,
    checklist,
    ...
    progress: computePlayStoreProgress(DASH_CUSTOMER_PLAY_STORE_CATALOG, checklist),
  };
}

export async function getDashPlayStoreLaunch(
  accessToken: string,
): Promise<PlayStoreLaunchPayload> {
  const res = await adminFetch(accessToken, BASE);
  if (!res.ok) throw new Error(await parseError(res));
  return mapPayload((await res.json()) as ApiPayload);
}

export async function patchDashPlayStoreChecklist(
  accessToken: string,
  patches: PlayStoreChecklistPatch[],
): Promise<Pick<PlayStoreLaunchPayload, 'checklist' | 'data_safety_notes' | 'progress'>> {
  const res = await adminFetch(accessToken, `${BASE}/checklist`, {
    method: 'PATCH',
...
export async function saveDashPlayStoreDataSafetyNotes(
  accessToken: string,
  notes: string,
...
export async function addDashPlayStoreRelease(
  accessToken: string,
  input: PlayStoreReleaseInput,
): Promise<PlayStoreReleaseRow> {
  const res = await adminFetch(accessToken, `${BASE}/releases`, {
    method: 'POST',
...
export async function deleteDashPlayStoreRelease(accessToken: string, id: string): Promise<void> {
  const res = await adminFetch(accessToken, `${BASE}/releases/${id}`, { method: 'DELETE' });
  if (!res.ok) throw new Error(await parseError(res));
}

export async function importDashPlayStoreDataSafetyCsv(
  accessToken: string,
  csv: string,
  dryRun = false,
): Promise<{
  ...
  const res = await adminFetch(accessToken, `${BASE}/data-safety/import`, {
    method: 'POST',
...
export async function exportDashPlayStoreDataSafetyCsv(accessToken: string): Promise<void> {
  const res = await adminFetch(accessToken, `${BASE}/data-safety/export`);
  if (!res.ok) throw new Error(await parseError(res));
...
export async function saveDashPlayStoreDataSafetyRows(
  accessToken: string,
  state: DataSafetyState,
  expectedUpdatedAt?: string | null,
): Promise<Partial<PlayStoreLaunchPayload>> {
  const res = await adminFetch(accessToken, `${BASE}/data-safety`, {
    method: 'PUT',
    body: JSON.stringify({
      rows: state.rows,
      templateVersion: state.templateVersion ?? DASH_CUSTOMER_DATA_SAFETY_TEMPLATE_VERSION,
