...
type ImportTownBorderOverlayProps = {
  ...
  onPromoteChange: (value: boolean) => void;
  onClose: () => void;
  onImport: () => void;
};

export function ImportTownBorderOverlay({
  open,
  kind,
  townName,
  text,
  ...
  const accept = kind === 'csv' ? '.csv,text/csv,text/plain' : '.geojson,.json,application/geo+json,application/json';
      ...
      >
        ...
        <div className="overflow-y-auto flex-1 p-4 space-y-3">
          ...
          <button
            ...
            className="w-full flex flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-slate-600 bg-slate-950/60 px-4 py-6 text-slate-300 hover:border-amber-500/50 hover:bg-slate-950"
          >
            <FileUp className="w-6 h-6 text-amber-300" />
            <span className="text-sm font-medium text-white">
              {kind === 'csv' ? 'Choose CSV file' : 'Choose GeoJSON file'}
            </span>
            <span className="text-[11px] text-slate-500">
              {kind === 'csv' ? '.csv' : '.geojson or .json'}
          ...
          <textarea
            ...
            placeholder={
              kind === 'csv'
                ? 'lat,lng\n18.0425,-76.9592\n17.9620,-76.9568\n…'
                : 'Paste GeoJSON Polygon / Feature, or [{"lat":…,"lng":…},…]'
            }
