  ...
  return (
    <header className="safe-t fixed top-0 left-0 z-50 flex h-16 w-full items-center justify-between border-b border-outline-variant bg-surface px-margin-mobile shadow-sm">
      <button
        type="button"
        onClick={onOpenNav}
        className={`btn-touch flex h-12 w-12 items-center justify-center rounded-full text-on-surface-variant transition-transform duration-200 hover:bg-surface-container active:scale-95 lg:hidden ${onOpenNav ? '' : 'invisible'}`}
