...
const fieldClass =
  'h-xl w-full rounded border border-outline-variant bg-surface-container-lowest px-sm text-body-lg text-on-surface placeholder:text-on-surface-variant/50 partner-field';

export default function BankDetailsStep({ data, onChange, onSave, onSkip }: BankDetailsStepProps) {
  const canSave =
    data.bankName !== '' &&
  ...
  return (
    <div className="flex min-h-dvh flex-col bg-background text-on-background">
      ...
      <main className="flex flex-grow flex-col items-center px-margin-mobile pb-24 pt-8 md:px-margin-tablet md:pt-16">
        <div className="w-full max-w-md rounded-lg border border-outline-variant bg-surface-container-lowest p-md shadow-[0px_4px_12px_rgba(0,0,0,0.05)]">
          <div className="mb-lg">
          ...
          <form className="space-y-md" onSubmit={(e) => e.preventDefault()}>
            ...
            <div className="flex flex-col gap-xs pt-xs">
              <span className="mb-base text-label-md font-semibold text-on-surface">Account Type</span>
              <div className="flex gap-gutter">
                {(['checking', 'savings'] as const).map((type) => (
                  <label
                    key={type}
                    className="flex flex-1 cursor-pointer items-center gap-xs rounded border border-outline-variant bg-surface-container-low p-sm transition-colors hover:bg-surface-container"
                  >
                    <input
                      type="radio"
                      name="accountType"
            ...
            <div className="mt-lg flex flex-col gap-sm border-t border-outline-variant pt-md">
              <button
                type="button"
                onClick={onSave}
                disabled={!canSave}
                className="flex h-xl w-full items-center justify-center rounded-full bg-primary-container text-label-md font-semibold text-on-primary-container shadow-sm transition-colors duration-150 hover:bg-primary-fixed active:scale-95 disabled:cursor-not-allowed disabled:opacity-50"
              >
                Save &amp; Continue
              </button>
              <button
                type="button"
                onClick={onSkip}
                className="flex h-xl w-full items-center justify-center rounded-full bg-transparent text-label-md font-semibold text-primary transition-colors hover:bg-surface-container-low"
              >
                Skip for now
              </button>
            </div>
          ...
          <div className="mt-lg flex items-start gap-xs rounded bg-surface-container-low p-sm text-on-surface-variant">
