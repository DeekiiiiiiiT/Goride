...
type Props = {
  open: boolean;
  onClose: () => void;
  onSelect: (tag: RoamPassengerTagBookingLookupDto) => void;
  title?: string;
  description?: string;
  confirmLabel?: string;
};

/** Search Roam by @tag and return a matched passenger profile. */
export function RoamTagLookupSheet({
  open,
  onClose,
  onSelect,
  title = 'Find by Roam Tag',
  description = 'Search for someone on Roam using their tag.',
  confirmLabel = 'Select',
}: Props) {
