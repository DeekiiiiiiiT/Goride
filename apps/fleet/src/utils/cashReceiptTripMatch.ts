/**
 * Soft-match cash / passage-receipt tolls to nearby trips that already carry
 * a platform toll credit (Uber / InDrive / Roam tollCharges), when the strict
 * trip-window matcher returned nothing.
 *
 * Why: cash receipts often lack plaza coords and sit just outside
 * request−45 → dropoff+15, so they become orphan PERSONAL instead of linking
 * to the obvious same-day trip credit (e.g. $380 cash vs $370 credit, ~20–40 min).
 */

/** Max minutes from toll to nearest trip request/dropoff for soft match. */
export const CASH_RECEIPT_TRIP_PROXIMITY_MINUTES = 90;

/**
 * Max |cash toll − trip.tollCharges| allowed (covers known tag/cash pairs
 * like Portmore 370/380 and Spanish Town 275/285).
 */
...
/**
 * Minutes from toll to the trip interval [start, end].
 * 0 if inside; otherwise distance to nearest endpoint.
 */
...
export interface CashReceiptSoftMatchInput {
  ...
  trips: Array<{
    ...
    date?: string | null;
    /** Pre-parsed times when caller already has them */
    tripStart?: Date | null;
    tripEnd?: Date | null;
  }>;
}
...
/**
 * Rank trips with platform toll credits that plausibly explain a cash receipt.
 * Empty when nothing qualifies — caller should fall through to orphan personal.
 */
export function findCashReceiptTripCreditHits(
  input: CashReceiptSoftMatchInput,
  ...
  for (const trip of input.trips) {
    ...
    // Base 58 — below typical ON_TRIP 70, above orphan soft scores; boosts for tightness.
    let score = 58;
    if (gap <= 15) score += 12;
