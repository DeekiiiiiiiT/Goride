/**
 * Pure Transaction Logs KPI roll-up — shared contract for client + server.
 * Edge mirror: supabase/functions/_fleet-server/fuel_log_summary.ts (Keep in sync).
 */
...
export type FuelLogSummaryTotals = {
  ...
  totalSpend: number;
  totalVolume: number;
  /** Fill-to-fill odometer deltas (not cycle distance). */
  totalKm: number;
  sourcePortal: number;
  sourceAdmin: number;
...
/** Keep in sync with jaaFuelStatementMatcher / fuel_jaa_ledger. */
...
/** Keep in sync with fuelOpsEligibility.countsInFuelLogSpend. */
...
/** Keep in sync with fuelEntrySource.resolveFuelEntrySource. */
export function resolveFuelEntrySourceCore(entry: FuelLogSummaryEntry): string {
  const m = metaOf(entry);
  const explicit =
    asAuthSource(entry.entrySource) || asAuthSource(m.entrySource);
...
/** Fill-to-fill odo deltas; skips JAA rows; positive deltas only. */
...
/**
 * Canonical Transaction Logs totals for a pre-scoped entry set.
 * Excludes JAA ledger from fills/km; spend/volume use ops eligibility.
 */
