...
export function processJaaRawFuelData(
  ...
  let rejectedForeignCompany = 0;
  const requireCardMatch = options?.requireCardMatch === true;
  const allowed = options?.allowedCompanyCodes;

  for (const row of rows) {
    const cardCode = getVal(row, ['CARD_CODE', 'Card Code', 'CardCode']);
    const companyCode = (getVal(row, ['COMPANY_CODE', 'Company Code']) || '').replace(/\D/g, '');
    const receiptNumber = getVal(row, ['RECEIPT_NUMBER', 'Receipt Number', 'Receipt']);

    if (allowed && allowed.size > 0) {
      if (!companyCode || !allowed.has(companyCode)) {
        rejectedForeignCompany++;
        continue;
      }
    }
    ...
    if (!matchedCard && requireCardMatch) {
      unmatchedRows.push({
        cardCode: String(cardCode || '').replace(/[^a-zA-Z0-9]/g, '').toUpperCase(),
        companyCode,
        receiptNumber: receiptNumber || '',
        amount: Math.abs(amountRaw),
        liters: liters > 0 ? liters : undefined,
        fuelAmount: fuelAmount > 0 ? fuelAmount : undefined,
        transDate: parsed.time ? `${parsed.date}T${parsed.time}` : parsed.date,
        vendor,
        fuelType,
        response,
        classification: kind,
    ...
    const entry: FuelEntry = {
      ...
      metadata: {
        ...
        jaaCompanyCode: companyCode || matchedCard?.jaaCompanyCode,
  ...
  return { entries, skippedDuplicates, unmatchedRows, rejectedForeignCompany };
}
