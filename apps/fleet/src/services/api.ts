...
export const api = {
  ...
  async deleteTransaction(id: string) {
    ...
    if (!response.ok) {
      ...
      const payload = {
        sessionId: '03bcb5',
        flowTag: 'fuel-delete-flow-v3',
        location: 'api.ts:deleteTransaction:fail',
        message: 'DELETE transaction non-OK',
        data: { httpStatus: st, txIdLen: trimmed.length, errMsg: (msg || '').slice(0, 240) },
        timestamp: Date.now(),
        hypothesisId: 'H2',
      };
