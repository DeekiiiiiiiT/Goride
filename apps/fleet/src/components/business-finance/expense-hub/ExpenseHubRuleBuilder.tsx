...
import type { ExpenseBulkPreview } from '../../../types/expenseHub';
...
export function ExpenseHubRuleBuilder({
  ...
  const boundaryValid = !endDate ||
    endDate > startDate ||
    (endDate === startDate && endTime > startTime);
  const detailsValid = name.trim().length > 0 && amountNum > 0 &&
    /^\d{4}-\d{2}-\d{2}$/.test(startDate) && timeValid && boundaryValid;
  ...
  React.useEffect(() => {
    setPreview(null);
    setPreviewError('');
  }, [amount, frequency, vehicleIds, startDate, startTime, endDate, endTime]);
  ...
  const runPreview = async () => {
    ...
    setPreviewError('');
    try {
      setPreview(await expenseHubService.previewRule({
        amount: amountNum,
        frequency,
        vehicleIds,
        startDate,
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.success(`Rule created for ${result.assignments.length} vehicle${result.assignments.length === 1 ? '' : 's'}`);
      reset();
      onOpenChange(false);
      onCreated?.();
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={(next) => !saving && onOpenChange(next)}>
      <DialogContent className="max-h-[96vh] gap-0 overflow-hidden p-0 sm:max-w-4xl">
        <DialogHeader className="border-b bg-slate-50 px-5 py-4 text-left dark:bg-slate-950 sm:px-6">
          ...
          <DialogDescription>Configure the cadence, assign vehicles, and verify the server forecast.</DialogDescription>
          <ol className="grid grid-cols-2 gap-2 pt-3" aria-label="Rule creation progress">
            {[
              { id: 1, label: 'Rule details' },
              { id: 2, label: 'Assign vehicles' },
            ].map((item) => (
              <li key={item.id}
          ...
          {step === 1 ? (
            <div className="grid gap-6 p-5 sm:p-6 lg:grid-cols-[minmax(0,1fr)_280px]">
              <section className="space-y-5">
                <div>
                  ...
                  <p className="text-sm text-slate-500">Set the recurring expense applied to each selected vehicle.</p>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="hub-rule-name">Rule name *</Label>
                  <Input id="hub-rule-name" value={name} onChange={(event) => setName(event.target.value)}
                    placeholder="e.g. Annual fleet insurance" className="h-11" />
                </div>
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    {!vendorsQuery.isLoading && !vendorsQuery.isError && categoryVendors.length === 0 && (
                      <p className="text-xs text-slate-500">No vendors are assigned to {category} yet.</p>
                    )}
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="hub-rule-category">Category *</Label>
                    <Select value={category} onValueChange={setCategory}>
                      <SelectTrigger id="hub-rule-category" className="min-h-11"><SelectValue /></SelectTrigger>
                      <SelectContent>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="space-y-2">
                    ...
                    <Input id="hub-rule-amount" type="number" min="0.01" step="0.01"
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={frequency} onValueChange={setFrequency}>
                      <SelectTrigger id="hub-rule-frequency" className="min-h-11"><SelectValue /></SelectTrigger>
                      <SelectContent>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <fieldset className="grid grid-cols-[minmax(0,1fr)_8rem] gap-2">
                    <legend className="sr-only">Coverage start</legend>
                    <div className="space-y-2">
                      <Label htmlFor="hub-rule-start">Start date *</Label>
                      <Input id="hub-rule-start" type="date" value={startDate}
                        onChange={(event) => setStartDate(event.target.value)} className="h-11" />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="hub-rule-start-time">Start time *</Label>
                      <Input id="hub-rule-start-time" type="time" value={startTime}
                        onChange={(event) => setStartTime(event.target.value)} className="h-11" />
                    </div>
                  </fieldset>
                  <fieldset className="grid grid-cols-[minmax(0,1fr)_8rem] gap-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input id="hub-rule-end" type="date" min={startDate} value={endDate}
                        onChange={(event) => setEndDate(event.target.value)} className="h-11" />
                    </div>
                    <div className="space-y-2">
                      <Label htmlFor="hub-rule-end-time">End time *</Label>
                      <Input id="hub-rule-end-time" type="time" value={endTime}
                        onChange={(event) => setEndTime(event.target.value)} className="h-11" />
                    </div>
                  </fieldset>
                </div>
                {!boundaryValid && (
                ...
                <label className="flex min-h-14 cursor-pointer items-center justify-between gap-3 rounded-lg border px-4">
                  <span><span className="block text-sm font-medium">Auto-renew</span><span className="block text-xs text-slate-500">Continue when no end date is set.</span></span>
                  <Switch aria-label="Auto-renew rule" checked={autoRenew} onCheckedChange={setAutoRenew} />
                </label>
                <div className="space-y-2">
                  <Label htmlFor="hub-rule-description">Internal description</Label>
                  <Textarea id="hub-rule-description" value={description}
                    onChange={(event) => setDescription(event.target.value)}
                    placeholder="Optional context for finance teams" rows={3} />
                </div>
              </section>
              <aside className="rounded-xl border bg-slate-50 p-5 dark:bg-slate-900/50">
                <Repeat2 className="h-7 w-7 text-indigo-600" />
                <h3 className="mt-4 font-semibold">One rule, many vehicles</h3>
                ...
                {amountNum > 0 && (
                  <div className="mt-5 rounded-lg bg-white p-4 dark:bg-slate-950">
                    ...
                    <p className="mt-1 text-xs capitalize text-slate-500">{frequency.replace('_', '-')}</p>
                  </div>
                )}
              </aside>
            </div>
            ...
            <div className="grid gap-6 p-5 sm:p-6 lg:grid-cols-[minmax(0,1fr)_300px]">
              <section className="space-y-4">
                <div>
                  <h3 className="font-semibold">Vehicle assignment</h3>
                  <p className="text-sm text-slate-500">Search the live fleet list and assign this rule in bulk.</p>
                </div>
                <VehicleMultiSelect selectedIds={vehicleIds} onChange={setVehicleIds} />
              </section>
              <aside className="space-y-4">
                <div className="rounded-xl border p-4">
                  <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Assignment summary</p>
                  <div className="mt-4 flex items-center gap-3">
                    <span className="rounded-lg bg-indigo-100 p-2 text-indigo-700 dark:bg-indigo-950"><Truck className="h-5 w-5" /></span>
                    <div><p className="text-2xl font-bold tabular-nums">{vehicleIds.length}</p><p className="text-xs text-slate-500">vehicles selected</p></div>
                  </div>
                  <dl className="mt-4 space-y-2 border-t pt-4 text-sm">
                    <div className="flex justify-between gap-3"><dt className="text-slate-500">Rule</dt><dd className="truncate font-medium">{name}</dd></div>
                    <div className="flex justify-between"><dt className="text-slate-500">Per vehicle</dt><dd className="font-medium tabular-nums">{formatMoney(amountNum)}</dd></div>
                    <div className="flex justify-between capitalize"><dt className="text-slate-500">Cadence</dt><dd>{frequency.replace('_', '-')}</dd></div>
                    <div className="flex justify-between gap-3"><dt className="text-slate-500">Effective</dt><dd className="text-right">{startDate} {startTime}</dd></div>
                    <div className="flex justify-between gap-3"><dt className="text-slate-500">Expires</dt><dd className="text-right">{endDate ? `${endDate} ${endTime}` : 'No end date'}</dd></div>
                  </dl>
                </div>
                <div className="rounded-xl border p-4">
                  <div className="flex items-center gap-2 font-semibold"><Calculator className="h-4 w-4 text-indigo-600" />Server preview</div>
                  <p className="mt-2 text-xs text-slate-500">Required before creating the rule.</p>
                  <Button type="button" variant="outline" className="mt-4 min-h-11 w-full"
                    disabled={!valid || previewing || saving} onClick={() => void runPreview()}>
                    ...
                    <div className="mt-4 space-y-3" aria-live="polite">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-center">
                        <div className="rounded-lg bg-slate-50 p-3 dark:bg-slate-900">
                          <p className="text-lg font-bold">{preview.includedVehicleIds.length}</p><p className="text-[11px] text-slate-500">Included</p>
                        </div>
                        <div className="rounded-lg bg-slate-50 p-3 dark:bg-slate-900">
                          <p className="text-lg font-bold">{preview.excludedVehicleIds.length}</p><p className="text-[11px] text-slate-500">Excluded</p>
                        </div>
                      </div>
                      <div className="rounded-lg bg-indigo-50 p-3 dark:bg-indigo-950/30">
                        ...
                        <p className="mt-1 text-lg font-bold tabular-nums">{formatMoney(preview.projectedAnnualTotal)}</p>
                        <p className="text-xs text-slate-500">{preview.estimatedOccurrenceCount} estimated occurrence{preview.estimatedOccurrenceCount === 1 ? '' : 's'}</p>
                      </div>
                    </div>
                  )}
                </div>
        ...
        <div className="flex flex-col-reverse gap-2 border-t bg-white px-5 py-4 dark:bg-slate-950 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          ...
          {step === 1 ? (
            <Button type="button" className="min-h-11" disabled={!detailsValid} onClick={() => setStep(2)}>
              Assign vehicles <ArrowRight className="ml-2 h-4 w-4" />
            </Button>
          ) : (
            <Button type="button" className="min-h-11" disabled={saving || !preview}
              onClick={() => void submit()}>
