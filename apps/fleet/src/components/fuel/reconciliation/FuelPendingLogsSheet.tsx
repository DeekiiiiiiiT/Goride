/**
 * Vehicle evidence for Data quality — flagged fills first, then pending.
 */
import {
  Sheet,
  SheetContent,
...
export function FuelVehicleEvidenceSheet({
  ...
  pendingEntries: FuelEntry[];
}) {
  const pendingOnly = pendingEntries.filter(
    (e) => !flaggedRows.some((r) => r.entryId === e.id),
  );
        ...
        <SheetHeader>
          ...
          <SheetDescription>
            {flaggedRows.length > 0
              ...
              : `Fills for ${driverName}`}
            {pendingOnly.length > 0
              ? ` · ${pendingOnly.length} pending will post on Finalize`
              : ''}
            . Close this panel to stay on reconciliation.
          </SheetDescription>
        </SheetHeader>

        {flaggedRows.length === 0 && pendingOnly.length === 0 ? (
          <p className="mt-6 rounded-lg border border-slate-200 bg-slate-50 px-4 py-8 text-center text-sm text-slate-600">
            No flagged or pending fills for this vehicle in the week window.
          </p>
        ) : (
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <p className="text-sm font-semibold text-slate-900">
                              {formatFuelLogDate(String(e.date || '').split('T')[0])}
                            </p>
                            <p className="mt-0.5 text-xs text-slate-500">
                              {humanizeEntryType(e.type)}
