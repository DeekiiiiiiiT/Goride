...
import type { ColumnDef } from './FuelLedgerTable';

interface FuelLedgerColumnToggleProps {
  columns: ColumnDef[];
...
export function FuelLedgerColumnToggle({
  columns,
  visibleColumns,
  onToggle,
  onResetDefaults,
}: FuelLedgerColumnToggleProps) {
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      >
        ...
        Columns
        {!isDefault && (
          <span className="ml-0.5 px-1.5 py-0.5 text-xs rounded-full bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-300">
            {activeCount}
          </span>
        )}
      </button>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-64 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-lg z-50">
          ...
          <div className="max-h-80 overflow-y-auto py-1">
            {(['core', 'audit', 'meta'] as const).map(group => {
                    ...
                    return (
                      ...
                      >
                        <input
                          ...
                          className="h-3.5 w-3.5 rounded border-slate-300 dark:border-slate-600 text-amber-600 focus:ring-amber-500 focus:ring-offset-0 cursor-pointer"
                        />
