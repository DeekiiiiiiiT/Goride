...
function TripLogsPageInner() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {/* Global Filter Bar — avoid -mx-6 on phone (overshoots ~1rem content pad) */}
      <div className="sticky top-0 z-10 border-b border-slate-200 bg-slate-50/95 py-2 shadow-sm backdrop-blur md:-mx-6 md:px-6">
           <TripFilters 
              filters={filters} 
      ...
      <Tabs defaultValue="manifest" className="w-full">
        <TabsList className="grid w-full max-w-[850px] grid-cols-2 sm:grid-cols-4">
          ...
          <Card className="border-slate-200 shadow-sm">
            ...
            <CardContent className="p-0">
                {tripsLoading ? (
                  ...
                  const visibleTrips = trips.filter(t => !searchTerm ||
                  ...
                  return (
                    <>
                      <div className="space-y-3 p-3 md:hidden">
                        {visibleTrips.map((trip) => {
                          const earnings = ((trip.platform === 'InDrive' && trip.indriveNetIncome != null) ? trip.indriveNetIncome : (trip.amount || 0)).toFixed(2);
                          return (
                            <div
                              key={trip.id}
                            ...
                            >
                              <button
                                ...
                                onClick={() => handleAction(trip, 'details')}
                              >
                                <div className="flex items-start justify-between gap-3">
                                  <div className="min-w-0">
                                    ...
                                    <p className="text-xs text-slate-500">
                                      {new Date(trip.requestTime || trip.date).toLocaleString([], {
                                ...
                                <div className="mt-3 grid grid-cols-2 gap-2 text-sm">
                                  ...
                                  <div>
                                    ...
                                    <p className="font-bold text-slate-900 dark:text-slate-100">${earnings}</p>
                                  </div>
                                  <div>
                                    <p className="text-xs text-slate-500">Distance</p>
                                    <p className="text-slate-700 dark:text-slate-300">{(trip.distance || 0).toFixed(1)} km</p>
                                  </div>
                                </div>
                              </button>
                    ...
                    <Table>
                      ...
                      <TableBody>
                          {visibleTrips.map((trip) => (
                            <TableRow key={trip.id} className="group hover:bg-slate-50/50">
                              <TableCell className="align-top">
                                <div className="flex flex-col">
                                    ...
                                    {trip.timeOfDay !== undefined && (
                                        <Badge variant="outline" className="mt-1 h-5 w-fit px-1 text-[10px] font-normal text-slate-400 border-slate-200">
                              ...
                              <TableCell className="align-top">
                                <div className="flex flex-col">
                                    ...
                                    <span className="mb-1 font-mono text-xs text-slate-500">
                                      {getTripVehicleLabel(trip, availableVehicles, availableDrivers)}
                                    </span>
                                    {trip.isLiveRecorded && (
                                        <Badge variant="outline" className="h-5 w-fit px-1 text-[10px] bg-indigo-50 text-indigo-700 border-indigo-200">
                                            Live Trip
                                        </Badge>
                                    )}
                                    {trip.isManual && !trip.isLiveRecorded && (
                                        <Badge variant="outline" className="h-5 w-fit px-1 text-[10px] bg-amber-50 text-amber-700 border-amber-200">
                                            Manual Entry
                                        </Badge>
                                    )}
                                    {trip.productType && !trip.isManual && !trip.isLiveRecorded && (
                                        <Badge variant="secondary" className="h-5 w-fit px-1 text-[10px] bg-slate-100 text-slate-600">
                                            {trip.productType}
                                        </Badge>
                                    )}
                                    {trip.serviceCategory === 'courier' && (
                                        <Badge variant="outline" className="h-5 w-fit px-1 text-[10px] bg-amber-50 text-amber-600 border-amber-300">
                                            📦 Courier
                                        </Badge>
                                    )}
                                    {(() => {
                                      ...
                                      if (coverage.status === 'detected') {
                                        return (
                                          <Badge variant="outline" className="h-5 w-fit px-1 text-[10px] bg-emerald-50 text-emerald-700 border-emerald-200">
                                      ...
                                      if (coverage.status === 'not_applicable') {
                                        return (
                                          <Badge variant="outline" className="h-5 w-fit px-1 text-[10px] bg-slate-50 text-slate-500 border-slate-200">
                                            No toll GPS
                                          </Badge>
                                        );
                                      }
                              ...
                              <TableCell className="max-w-[300px] align-top">
                                <div className="flex flex-col gap-1">
                                    <div className="flex items-start gap-2 text-sm">
                                        ...
                                        <span className="truncate text-slate-700" title={getTripEndpointLabel(trip.pickupLocation, trip.pickupArea, getTripStartCoords(trip), 'Unknown Pickup')}>
                                            {getTripEndpointLabel(trip.pickupLocation, trip.pickupArea, getTripStartCoords(trip), 'Unknown Pickup')}
                                        </span>
                                    </div>
                                    <div className="ml-0.5 h-3 border-l-2 border-slate-100 pl-0.5" />
                                    <div className="flex items-start gap-2 text-sm">
                                        <div className="mt-1 h-2 w-2 shrink-0 rounded-full bg-rose-500" />
                                        <span className="truncate text-slate-700" title={getTripEndpointLabel(trip.dropoffLocation, trip.dropoffArea, getTripEndCoords(trip), 'Unknown Dropoff')}>
                                            {getTripEndpointLabel(trip.dropoffLocation, trip.dropoffArea, getTripEndCoords(trip), 'Unknown Dropoff')}
                                        </span>
                                    </div>
                                </div>
                              ...
                              <TableCell className="align-top text-center">
                                ...
                                {trip.status === 'Cancelled' && trip.cancelledBy && (
                                  <p className="mt-0.5 capitalize text-[10px] text-rose-500">
                                    by {trip.cancelledBy}
                                  </p>
                                )}
                              </TableCell>
