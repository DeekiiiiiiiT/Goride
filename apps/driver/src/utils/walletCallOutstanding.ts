/**
 * Call-script / Cash Settlement amounts — Settlement SSOT, display only.
 * Copy of apps/fleet/src/utils/walletCallOutstanding.ts — keep in sync.
 */
  ...
  if (row.isFinalized) {
    ...
    if (s < -0.005) {
      return {
        ...
        callLabel: 'You still owe the fleet',
        breakdown,
      };
    }
    if (s > 0.005) {
