...
export function DriverHomeDashboard({ tripFlowActive }: Props) {
  const { data, loading, error, refresh } = useIndependentEarnings('today');
  ...
  const earningsAmount =
    data != null
      ? formatStatAmount(data.cash_minor + data.digital_minor, data.currency)
      : loading
        ? null
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col px-6 pb-6 pt-2">
      ...
      <div className="driver-home-premium-card rounded-3xl p-4 shadow-2xl">
        <OnlineGaugeSlider
          variant="premium"
          online={online}
