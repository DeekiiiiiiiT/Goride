...
import {
  ...
  DRIVER_DATA_SAFETY_TEMPLATE_VERSION,
...
export async function importDriverPlayStoreDataSafetyCsv(
  accessToken: string,
  csv: string,
  dryRun = false,
): Promise<{
  ...
  const res = await adminFetch(accessToken, `${DRIVER_BASE}/admin/play-store/data-safety/import`, {
    method: 'POST',
...
export async function exportDriverPlayStoreDataSafetyCsv(accessToken: string): Promise<void> {
  const res = await adminFetch(accessToken, `${DRIVER_BASE}/admin/play-store/data-safety/export`);
  if (!res.ok) throw new Error(await parseError(res));
...
export async function saveDriverPlayStoreDataSafetyRows(
  accessToken: string,
  state: DataSafetyState,
  expectedUpdatedAt?: string | null,
): Promise<Partial<PlayStoreLaunchPayload>> {
  const res = await adminFetch(accessToken, `${DRIVER_BASE}/admin/play-store/data-safety`, {
    method: 'PUT',
    body: JSON.stringify({
      rows: state.rows,
      templateVersion: state.templateVersion ?? DRIVER_DATA_SAFETY_TEMPLATE_VERSION,
