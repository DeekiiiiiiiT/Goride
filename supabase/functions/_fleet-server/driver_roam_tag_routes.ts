/**
 * Driver-only Roam Tags — ensure / me / claim / lookup.
 * Separate namespace from courier delivery.courier_roam_tags and passenger tags.
 */
...
function generateInternalDriverTagId(): string {
...
async function insertInternalTag(
  ...
  for (let attempt = 0; attempt < 12; attempt++) {
    const internalTagId = generateInternalDriverTagId();
...
async function ensureDriverTag(
  supabase: SupabaseClient,
  userId: string,
): Promise<Record<string, unknown>> {
  const { data: existing } = await supabase
...
export function registerDriverRoamTagRoutes(
  app: Hono,
  deps: {
    supabase: SupabaseClient;
    requireAuth: () => unknown;
  ...
  const base = "/make-server-37f42386/driver-roam-tag";
  ...
  app.post(`${base}/ensure`, deps.requireAuth() as never, async (c) => {
    try {
      ...
      const row = await ensureDriverTag(deps.supabase, rbac.userId);
  ...
  app.patch(`${base}/me`, deps.requireAuth() as never, async (c) => {
    try {
      ...
      await ensureDriverTag(deps.supabase, rbac.userId);
      ...
      const { data: taken } = await deps.supabase
        .from("driver_roam_tags")
        .select("user_id")
  ...
  /** Fleet owner lookup — resolves driver @tag for invite targeting. */
  app.get(`${base}/lookup/:name`, deps.requireAuth() as never, async (c) => {
    try {
      ...
      const { data: tagRow, error } = await deps.supabase
        .from("driver_roam_tags")
