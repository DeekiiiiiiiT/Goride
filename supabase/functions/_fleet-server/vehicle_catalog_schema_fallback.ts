...
/**
 * PostgREST: column missing from **schema cache** (not necessarily missing from Postgres).
 * Never strip INSERT payload keys for these errors — that produces rows with NULL in columns that exist.
 */
export function isPostgrestVehicleCatalogSchemaCacheError(err: { message?: string; code?: string } | null): boolean {
  if (!err) return false;
...
/** Only strip optional columns on insert retry for real missing-column errors — not stale PostgREST cache. */
export function shouldStripVehicleCatalogInsertPayloadOnRetry(err: { message?: string; code?: string } | null): boolean {
  return isVehicleCatalogSchemaMismatchError(err) && !isPostgrestVehicleCatalogSchemaCacheError(err);
}
...
/** Fuel economy columns (`20260501120000_*`); strip when DB is behind migration. */
const VEHICLE_CATALOG_FUEL_ECONOMY_KEYS = ["fuel_economy_km_per_l", "estimated_km_per_refuel"] as const;

export function stripVehicleCatalogFuelEconomyColumns(row: Record<string, unknown>): Record<string, unknown> {
  const out = { ...row };
  for (const k of VEHICLE_CATALOG_FUEL_ECONOMY_KEYS) {
    delete out[k];
  }
  return out;
}
...
/** Motorcycle class migration (`20260914010000_*`); strip when DB is behind. */
const VEHICLE_CATALOG_MOTORCYCLE_KEYS = [
  "vehicle_class",
  "final_drive",
  "cooling_type",
  "starter_type",
...
export function stripVehicleCatalogMotorcycleColumns(row: Record<string, unknown>): Record<string, unknown> {
  const out = { ...row };
  for (const k of VEHICLE_CATALOG_MOTORCYCLE_KEYS) {
    delete out[k];
  }
  return out;
}
...
export function stripVehicleCatalogOptionalMigrationColumns(row: Record<string, unknown>): Record<string, unknown> {
  return stripVehicleCatalogMotorcycleColumns(
    stripVehicleCatalogSpecPackColumns(
      stripVehicleCatalogCsvAlignmentColumns(stripVehicleCatalogFuelEconomyColumns(row)),
    ),
  );
}
...
/**
 * Explicit column list for PostgREST (avoids empty fields in `.select()` / `*` when the schema
