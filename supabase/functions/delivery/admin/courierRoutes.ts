...
export function registerCourierAdminRoutes(app: Hono) {
  ...
  admin.get("/orders/:orderId", async (c) => {
    ...
    const { data: order, error } = await db.from("orders")
      .select(`
        *,
        merchant:merchants(id, name, phone, address),
    ...
    if (order.courier_id) {
      const { data: cp } = await db.from("courier_profiles")
        .select("display_name")
        .eq("user_id", order.courier_id as string)
        .maybeSingle();
  ...
  admin.post("/orders/:orderId/cancel", async (c) => {
    const adminUser = c.get("adminUser") as ProductAdminUser;
    const denied = requireWrite(adminUser);
    if (denied) return denied;
    ...
    await courierAudit(adminUser.id, "admin_order_cancelled", { order_id: orderId, reason });

    return c.json({ ok: true, order });
  });

  admin.post("/orders/:orderId/complete", async (c) => {
    const adminUser = c.get("adminUser") as ProductAdminUser;
    const denied = requireWrite(adminUser);
    if (denied) return denied;
    ...
    await courierAudit(adminUser.id, "admin_order_completed", { order_id: orderId });
