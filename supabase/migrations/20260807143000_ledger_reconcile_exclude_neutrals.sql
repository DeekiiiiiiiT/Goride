-- Exclude intentional non-money rows from island legacy counts so Dominion
-- "green" matches dual-write policy (skip neutrals / zero amounts).
...
AS $$
  ...
  legacy AS (
    -- Money FE rows only (zero/neutral markers are not posted to unified)
    ...
    UNION ALL
    -- Money KV canonical events only (distinct dual-write idempotency keys)
    ...
    UNION ALL
    -- Reporting-only island (dual-write intentionally not mirrored)
