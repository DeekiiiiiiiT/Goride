-- Book-for-someone-else: guest passenger contact on ride_requests.

ALTER TABLE rides.ride_requests
  ADD COLUMN IF NOT EXISTS guest_passenger_name TEXT,
  ADD COLUMN IF NOT EXISTS guest_passenger_phone TEXT,
  ADD COLUMN IF NOT EXISTS booking_purpose TEXT CHECK (
    booking_purpose IS NULL OR booking_purpose IN ('guest', 'family', 'business')
  );

COMMENT ON COLUMN rides.ride_requests.guest_passenger_name IS
  'Display name of the person riding when booked by another account holder.';
COMMENT ON COLUMN rides.ride_requests.guest_passenger_phone IS
  'E.164 phone for SMS ride updates to the guest passenger.';
COMMENT ON COLUMN rides.ride_requests.booking_purpose IS
  'Why the account holder booked for someone else: guest, family, or business.';
