-- Y-1: provenance for pause threshold — seeded (market/pricing default) vs override (desk).
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'courier_remittance_accounts_threshold_source_check'
      AND conrelid = 'delivery.courier_remittance_accounts'::regclass
  ) THEN
    ALTER TABLE delivery.courier_remittance_accounts
...
UPDATE delivery.courier_remittance_accounts
SET threshold_source = 'seeded'
WHERE threshold_source IS DISTINCT FROM 'override';

-- New accounts created by apply_remittance_event are always seeded.
CREATE OR REPLACE FUNCTION delivery.apply_remittance_event(
  ...
  p_pause_threshold_minor bigint DEFAULT NULL
...
BEGIN
  ...
  v_threshold := coalesce(nullif(p_pause_threshold_minor, 0), 1000000);

  INSERT INTO delivery.courier_remittance_accounts (
    courier_id, pause_threshold_minor, threshold_source
  )
  VALUES (p_courier_id, v_threshold, 'seeded')
...
REVOKE ALL ON FUNCTION delivery.apply_remittance_event(
  uuid, text, bigint, text, uuid, bigint, bigint, bigint, bigint,
...
GRANT EXECUTE ON FUNCTION delivery.apply_remittance_event(
  uuid, text, bigint, text, uuid, bigint, bigint, bigint, bigint,
  uuid, uuid, uuid, text, text, jsonb, bigint
) TO service_role;
