-- EXC-7: ST_Difference often returns Polygon; column is MultiPolygon.
...
BEGIN
  ...
  -- ST_Difference may return Polygon; column is MultiPolygon — normalize.
  v_net := ST_Multi(
  ...
  IF v_net IS NULL OR ST_IsEmpty(v_net) THEN
    UPDATE delivery.service_markets
    SET net_coverage_geom = NULL,
        net_coverage_stats = jsonb_build_object(
          'net_sqkm', 0,
