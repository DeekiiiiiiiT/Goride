-- Phase B–E prep: real Dash legacy counts + soak status helper.
-- dash_payments legacy_count was previously hardcoded to 0 (always-fake green).
  ...
  legacy AS (
    ...
    SELECT 'rides_payment_journal',
           (SELECT count(*)::bigint
            ...
            WHERE abs(coalesce(amount_minor, 0)) > 0)
    UNION ALL
    -- Money Dash rows that dual-write posts (one receipt per source_id)
...
-- Soak / Phase-0 go-no-go snapshot for Dominion + scripts
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'instructions', jsonb_build_object(
      ...
      'allowed_skips', jsonb_build_array('neutral','zero_amount','missing_organization_id','self_ref_accounts')
    )
  );
END;
$$;

REVOKE ALL ON FUNCTION public.ledger_soak_status() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ledger_soak_status() TO service_role;
