-- Fix: rides_patch_ride_request ignored payment_method + completed_at on driver completion.
-- Hosted PostgREST uses this RPC; edge patch was dropped and earnings/week totals diverged.
...
-- Repair rows already marked completed without ledger fields
UPDATE rides.ride_requests
SET
  payment_method = COALESCE(payment_method, 'cash'),
  completed_at = COALESCE(completed_at, updated_at, created_at)
WHERE status = 'completed'
  AND (payment_method IS NULL OR completed_at IS NULL);
