-- X-3: seed pause_threshold_minor from market rules on first account create.
-- Existing accounts are never overwritten (ON CONFLICT DO NOTHING).
-- Drop prior 15-arg signature before adding p_pause_threshold_minor.
DROP FUNCTION IF EXISTS delivery.apply_remittance_event(
...
BEGIN
  ...
  INSERT INTO delivery.courier_remittance_accounts (courier_id, pause_threshold_minor)
  VALUES (p_courier_id, v_threshold)
...
-- Y-3: PostgREST caches RPC signatures; reload after arity change.
