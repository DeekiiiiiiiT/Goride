...
-- no INSERT/UPDATE/DELETE policy â€” ledger is system-written only
```
...
- `rides.service_body_types` never got RLS enabled â€” same latent shape as `ledger_lines`.
- A dozen `SECURITY DEFINER` RPCs (`rides_create_ride_request`, `rides_patch_ride_request`, `rides_cancel_ride_request`, driver-offer functions) have zero internal ownership checks â€” safety is 100% dependent on `GRANT EXECUTE` scoping, and that scoping already slipped once. Recommend a CI check asserting these grants never include `authenticated`/`anon`.
...
- `ride_requests`/`driver_offers` writes are never granted directly to `authenticated` â€” only SELECT; all mutation goes through service-role-gated RPCs.
...
## G5 â€” Rides: pin verification, live tracking, geofence, scheduled rides, dispatch

30 migrations. Same "unfiltered view" bug as G4, this time on live driver location and the ride PIN.

### ðŸš¨ Critical

**`public.rides_driver_locations` bypasses RLS â€” every driver's live GPS exposed to every signed-in user.** (`haul_dispatch_mode.sql:68-72`, `h3_supply_index.sql:79-92`.) The underlying table's policy is correctly scoped to `user_id = auth.uid()`, but the view ignores it. The first version of this view even granted `SELECT` to `anon` â€” fully public â€” before a later migration quietly dropped that grant without ever fixing the actual bypass.
```sql
...
- `rides.ride_location_updates`, `rides.ride_live_state`, and `rides.ride_toll_crossings` never got RLS enabled â€” same latent per-second-GPS-trail exposure risk.
- The omnibus `rides_patch_ride_request`/`..._cas` RPC has no column allow-list â€” it can set fare, PIN, and driver assignment via one JSONB blob. Safe only because it's `service_role`-only today.

### ðŸ§¹ Cleanup & performance

`rides_ride_requests` and `rides_dispatch_settings` are dropped and recreated fresh in nearly every migration in this batch, hand-copying the grant list from memory each time â€” that's exactly how `anon` got added to `rides_driver_locations` and had to be walked back five days later.
...
- The base-table RLS on `driver_locations` and `ride_requests` is itself well-designed â€” the security failure is entirely in the unfiltered views layered on top.

---

## G6 â€” Passenger social: contacts, connections, trip intents, saved places
...
**Nine tables holding contacts, live-location-share tokens, delegated-booking data, and the social graph never got RLS enabled â€” ever.** Highest-stakes: `ride_trip_shares` (the token that gates who can see a rider's live location) and `booking_requests` (delegated/Roam-Tag booking, including a phone number field).
```sql
...
- `fix_targeted_trip_intent_audience.sql` is a data backfill, not a policy fix â€” "targeted" vs. "open" audience has zero database-level enforcement today, because the table it governs has no RLS at all.
- The `rides_create_ride_request` RPC trusts caller-supplied `rider_contact_id`/`rider_user_id` with no ownership check â€” safe today only because it's `service_role`-only.
...
## G7 â€” Cash settlement, wallets, ledger core
...
**Independent of the view bug, the policy itself leaks personal wallets.** (`ledger_schema_core.sql:379-416`.) `organization_id IS NULL` is the first clause in an OR chain on all three SELECT policies â€” functionally `USING (true)` for the entire consumer ledger.
```sql
...
**A third recreation of `public.rides_ride_requests`** (`cash_settlement_wallet.sql:87-91`) repeats the same unfiltered-view bug in the migration that adds cash/tip fields â€” leaking the exact data it introduces.
...
- `ledger_scoped_views.sql` is the correct pattern the broken views above should have used â€” every one of its four views explicitly declares `security_invoker = true`.

---

## G8 â€” Toll/fuel brain, platform RBAC, enterprise inventory, haulage

18 migrations. Includes `platform_rbac_schema.sql` â€” the single highest-leverage file in the whole audit, since every other table's admin check ultimately traces back to it.

### ðŸš¨ Critical

**Two inventory-ledger RPCs are callable by a fully unauthenticated caller.** (`enterprise_inventory_rpcs.sql:3-157`.) `inventory_append_entry_tx` and `receive_purchase_order_tx` have no explicit `GRANT`/`REVOKE`, so they auto-inherit `EXECUTE` from an early blanket `ALTER DEFAULT PRIVILEGES ... TO anon, authenticated` on the whole `delivery` schema. Neither function checks the caller. Only stopped today because the tables they write to have RLS enabled with zero policies â€” an accident, not a designed control.
```sql
...
- `toll.brain_policies` never got RLS enabled, exposed via a `public.toll_brain_policies` view granted to `authenticated` â€” every row visible platform-wide. The sibling `fuel.brain_policies` table got this exactly right.
```sql
...
```
- `public.toll_settlement_allocations` â€” a financial table â€” never got RLS enabled either, safe today only because no client-facing grant exists yet.
- Enterprise inventory has RLS enabled on all 22 tables but a policy defined for only one (`inventory_nodes`) â€” tenant isolation is effectively unimplemented at the database layer for this feature.
...
- **`platform.user_roles` â€” the root of the entire role system â€” is correctly locked down.** INSERT/UPDATE/DELETE all gate on a platform-owner check, and UPDATE has both `USING` and `WITH CHECK`. No user can grant themselves a role through this table.
...
- The `fuel` schema's RLS is done consistently right across all four of its tables â€” the template `toll.brain_policies` should have copied.

---

*Compiled from eight independent file-by-file passes tracking every migration to its effective final state, not first-seen. Every finding cites a specific file and line, and every fix is a copy-pasteable `DROP POLICY`/`CREATE POLICY` (or `ALTER TABLE`/`ALTER VIEW`) statement â€” run them in order, and re-test the specific self-approval / self-escalation scenario described before considering a finding closed.*

---

## Remediation status (Waves 0â€“4)
...
| `organization_id IS NULL OR â€¦` wallet leak | Critical | **Fixed** | Wave 2 | Owner/driver/org-owner checks only |
...
| Latent tables without RLS (ledger_lines, social, toll brain, settlement, â€¦) | Critical/High | **Fixed** | Wave 4 | ENABLE RLS + participant/service policies |
| `evidence_files` USING (true) | High | **Skipped on prod** | Wave 4 | Table not present on production; guarded in migration |

**Migrations:** `20260718160000`â€“`20260718164000` (plus Wave 3a/3b/4a/4b applied via Supabase MCP).  
