...
2. Your **website on Vercel** needs two public pieces of information to talk to Supabase:
   - **Where** Supabase lives (the **Project URL**).
   - **Which browser key** to use (the **anon / public** key — *not* the secret admin key).
...
2. **Name (Key):** `VITE_SUPABASE_URL`  
   - Type this **exactly**. Capital letters matter.
3. **Value:** paste your **Project URL** from Step 0 (example: `https://xxxxx.supabase.co`).
4. **Environments:** enable **Production**.  
   - Also enable **Preview** if you want preview links to work with Supabase.
5. Click **Save**.
...
- **Local development** URLs if you test on your computer, for example:
  - `http://localhost:3000/**`
  - `http://localhost:5174/**` (port depends on your app)
...
- Supabase Auth redirect URLs: [https://supabase.com/docs/guides/auth/redirect-urls](https://supabase.com/docs/guides/auth/redirect-urls)  
...
- Vite env variables (`VITE_` prefix): [https://vitejs.dev/guide/env-and-mode.html](https://vitejs.dev/guide/env-and-mode.html)
