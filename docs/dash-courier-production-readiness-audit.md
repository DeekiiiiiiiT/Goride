...
> **Remediation status (2026-07-29):** Soft-launch program implemented in code + migration `20260729140000_dash_courier_soft_launch.sql` applied to GoRide. Courier orders RLS, `assigned` status machine, RealDispatchProvider, availability/GPS transmit, documents bucket, age-verify photo audit, courier_offers + POD/issues/earnings/payouts routes are in place. Deploy `delivery` + `notifications` edge functions and dogfood on staging before treating as production-complete.
