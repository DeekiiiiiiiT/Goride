...
> **2026-07-29 update (dash-merchant):** `ImageUpload.tsx` and `EditProfileView.tsx` now upload exclusively via `uploadMerchantAsset()` ? `POST /merchant-assets/upload` (JWT + magic-byte MIME validation + UUID paths). The older ?direct browser write to `merchant-assets`? finding below is **stale for dash-merchant** and closed. Historical Critical write-path context is retained for other apps (haul driver photos/documents) still using direct Storage writes.
...
> **Update 2026-07-29:** This finding is **stale for dash-merchant**. Both `ImageUpload.tsx` and `EditProfileView.tsx` now upload exclusively via `uploadMerchantAsset()` ? `POST /merchant-assets/upload` (edge function with JWT, magic-byte MIME validation, 5MB limit, UUID-scoped paths). There is no remaining direct `supabase.storage.from(...).upload` path in the merchant app. Keep the historical description above for audit trail only.
