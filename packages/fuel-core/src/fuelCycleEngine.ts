import type { FuelEntry, FuelCycle, FuelCalcVehicle } from './fuelTypes.ts';
import { classifyAnchor, isStableCycleId, resolveTankCapacity } from './fuelAnchorLogic.ts';
import {
    evaluateCycleClose,
 ...
 */
export function calculateFuelCycles(entries: FuelEntry[], vehicles: FuelCalcVehicle[] = []): FuelCycle[] {
    if (!entries || entries.length === 0) return [];

    const vehicleMap = new Map<string, FuelCalcVehicle>();
                ...
                if (lastAnchorOdometer !== undefined) {
                    ...
                    if (distance > 0) {
                        ...
                        const persistedCycleId = isStableCycleId(meta.cycleId)
                            ? String(meta.cycleId)
                            : cycleTransactions
                                .map((t) => t.metadata?.cycleId)
                                .find((id) => isStableCycleId(id));
                        const cycleId =
                          (typeof persistedCycleId === 'string' && persistedCycleId) ||
                          `cycle_${entry.id}_${index}`;
