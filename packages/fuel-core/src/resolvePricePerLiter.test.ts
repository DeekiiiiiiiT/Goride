...
describe('resolvePricePerLiter', () => {
  ...
  it('marks unavailable for cash-only / missing import (no invent)', () => {
  ...
  it('marks unavailable when cost missing but liters present', () => {
    const r = resolvePricePerLiter({ totalLiters: 10, totalGasCardCost: 0 });
    expect(r.priceUnavailable).toBe(true);
  });
});
