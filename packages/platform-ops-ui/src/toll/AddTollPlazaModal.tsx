...
import { TollPlaza, TollDirection, TollPlazaStatus } from '@roam/types/toll';
import {
  encodePlusCode,
  decodePlusCode,
  isValidPlusCode,
...
export function AddTollPlazaModal({ isOpen, onClose, onSaved, editingPlaza }: AddTollPlazaModalProps) {
    ...
    try {
      ...
      if (isFullPlusCode(codePart)) {
        ...
        try {
          ...
          const refResult = await api.geocodeAddress(localityQuery);
  ...
  const handleVerifyFromAddress = async () => {
    ...
    try {
      ...
      const result = await api.geocodeAddress(fullQuery);
