...
import {
  ...
  mergeSettings,
  defaultSettingsForSegment,
  defaultConsumerSegmentSettings,
  type FleetProductSettings,
...
// Types
// -------------------------------------------------------------------
export type ProductLineSettingsPageProps = {
...
function defaultForSegment(segment: SettingsSegment): SettingsData {
  if (segment === 'fleet') return { ...DEFAULT_FLEET_SETTINGS };
  if (segment === 'enterprise') return { ...DEFAULT_ENTERPRISE_SETTINGS };
  if (segment === 'rides' || segment === 'driver' || segment === 'haul' || segment === 'dash') {
    return defaultConsumerSegmentSettings(segment);
  }
  return defaultSettingsForSegment(segment) as SettingsData;
}

function isProductLineSegment(segment: SettingsSegment): segment is ProductLineSegment {
...
// Component
// -------------------------------------------------------------------
export function ProductLineSettingsPage({
  segment,
  apiBaseUrl,
  accessToken,
  userEmail,
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${apiBaseUrl}/admin/platform-settings`, {
        headers: withSettingsSegmentHeaders(segment, {
      ...
      if (data.segment) setResolvedSegment(data.segment);
      if (data.settings) {
        setSettings(mergeSettings(segmentDefaults, data.settings));
      }
    } catch (err: any) {
  ...
  }, [accessToken, segment, segmentDefaults]);
  ...
  const toggleModule = (key: string) => {
    ...
    const mods = settings.enabledModules as Record<string, boolean>;
    setSettings(prev => ({
      ...prev,
      enabledModules: {
        ...prev.enabledModules,
    ...
    <div className="space-y-6 max-w-3xl">
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <ReadOnlyField label="Super Admin" value={userEmail || '—'} />
          <ReadOnlyField label="Admin Role" value={userRole || 'superadmin'} />
          <ReadOnlyField label="Settings Segment" value={resolvedSegment} />
          <ReadOnlyField label="Platform" value={platformLabel} />
      ...
      >
        ...
        <SecurityEmergencyActions accessToken={accessToken} apiBaseUrl={apiBaseUrl} />
      </SettingsSection>
      </>}

      {/* ── Tab: Features ── */}
      {activeTab === 'features' && showFeaturesTab && <>
      {/* ── Section: Modules ── */}
      <SettingsSection
      ...
      <DangerZoneSection accessToken={accessToken} apiBaseUrl={apiBaseUrl} segment={segment} settings={settings} setSettings={setSettings} setSaving={setSaving} setSaved={setSaved} setError={setError} setDirty={setDirty} />
      </>}
    </div>
  );
}
...
function RegistrationSection({ settings, updateField, setSettings, setDirty }: {
  ...
  updateField: (field: keyof SettingsData, value: any) => void;
...
// Danger Zone Section
// ===================================================================
function DangerZoneSection({ accessToken, apiBaseUrl, segment, settings, setSettings, setSaving, setSaved, setError, setDirty }: {
  accessToken?: string;
  apiBaseUrl: string;
  segment: SettingsSegment;
  settings: SettingsData;
  ...
  const authHeaders = withSettingsSegmentHeaders(segment, {
    'Content-Type': 'application/json',
    Authorization: `Bearer ${accessToken}`,
  });
  ...
  const handleExport = async () => {
    ...
    try {
      const res = await fetch(`${apiBaseUrl}/admin/export-data`, { headers: authHeaders });
      const data = await res.json();
  ...
  const handleHealthCheck = async () => {
    ...
    try {
      const res = await fetch(`${apiBaseUrl}/admin/system-health`, { headers: authHeaders });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
      setHealthData(data);
    } catch (e: any) {
  ...
  const handlePurge = async () => {
    ...
    setActionResult(null);
    try {
      const res = await fetch(`${apiBaseUrl}/admin/purge-synthetic`, { method: 'POST', headers: authHeaders });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
  ...
  const handleReset = async () => {
    ...
    try {
      const res = await fetch(`${apiBaseUrl}/admin/platform-settings`, {
        method: 'PUT',
        headers: authHeaders,
        body: JSON.stringify(defaultForSegment(segment)),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || `HTTP ${res.status}`);
      setSettings(defaultForSegment(segment));
      setDirty(false);
...
// ===================================================================
function SecurityEmergencyActions({ accessToken, apiBaseUrl }: { accessToken?: string; apiBaseUrl: string }) {
  ...
  const handleTerminateAll = async () => {
    ...
    setResult(null);
    try {
      const res = await fetch(`${apiBaseUrl}/admin/terminate-all-sessions`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${accessToken}`,
