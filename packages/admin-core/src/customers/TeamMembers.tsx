...
// Helpers
// -------------------------------------------------------------------
function recoverySurfaceForTeam(
  apiNamespace: TeamMembersProps['apiNamespace'],
  productLine: 'enterprise' | 'fleet',
): AuthRecoverySurface {
  if (apiNamespace === '/enterprise-admin') return 'enterprise';
  if (apiNamespace === '/admin') return 'dominion';
...
export function TeamMembers({
  ...
  const { canEdit, canResetPassword, canSuspend, canManageTeam } = deriveAccountCapabilities(
    callerRole,
    apiNamespace,
  );
  const recoverySurface = recoverySurfaceForTeam(apiNamespace, productLine);
  ...
  const { data: members = [], isLoading: loading, error: queryError, refetch } = useQuery({
    queryKey: ['adminTeamMembers', productLine, apiNamespace],
    queryFn: async () => {
      const qs = new URLSearchParams({ productLine });
      const res = await fetch(`${api.listTeamMembers}?${qs.toString()}`, {
  ...
  const handleChangeRoleSubmit = async () => {
    ...
    const roleUrl = api.teamMemberRole(changeRoleMember.id);
    if (!roleUrl) return;
  ...
  const handleRemove = (member: TeamMember) => {
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        const deleteUrl = api.teamMemberDelete(member.id);
        if (!deleteUrl) {
