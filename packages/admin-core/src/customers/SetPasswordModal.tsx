...
export function SetPasswordModal({
  ...
  const applyPassword = async (pw: string) => {
    setLoading(true);
    try {
      const res = await fetch(setPasswordUrl, {
        method: 'POST',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-white border border-slate-300 dark:bg-slate-900 dark:border-slate-700 rounded-xl shadow-2xl w-full max-w-sm p-6 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button onClick={handleClose} className="p-1 text-slate-400 hover:text-slate-900 dark:hover:text-white">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400">
          ...
          <span className="text-slate-900 dark:text-white font-medium">{userName || userEmail}</span>
        </p>
