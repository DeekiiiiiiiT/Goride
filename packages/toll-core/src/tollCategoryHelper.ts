 ...
 * Fleet-canonical: isTollCategory = usage only; isTollLedgerCategory includes credits.
 */
...
export function tollLogKindFromTx(tx: { type?: string | null; category?: string | null; amount?: number }): TollLogKind {
  ...
  if (typeof tx.amount === 'number' && tx.amount > 0) return 'top-up';
  return 'usage';
}
